...
public class FileUtil {
  ...
  private static Optional<Path> searchPaths(String xdgPathSpecification, String target) {
    System.out.println("Coming in with " + xdgPathSpecification + " which is split as " + Arrays.toString(xdgPathSpecification.split(":")));
    for (String path : xdgPathSpecification.split(":")) {
      if (path.isEmpty()) {
        continue;
      }
  ...
  public static Optional<Path> findFolderInDatapath(String folder) {
    if (SystemUtils.IS_OS_WINDOWS) {
      return searchPaths(Path.of(System.getenv("APPDATA"), APPNAME, "data").toAbsolutePath().toString(), folder);
    } else if (SystemUtils.IS_OS_UNIX) {
      ...
      return searchPaths(String.format("%s:%s", xdgDataHome, xdgDataDirs), folder);
    } else {
      throw new NotImplementedException("Only defined data dirs for windows and unix-like (XDG) OS, so far");
    }
  }
